              ...
              return (
                <div class="col-6 col-xl-3 mx-auto">
                  <div class="card mb-3 cardSize" key={i}>
                    <img class="card-img" src={`/assets/snacks/0/Gob/${i}.jpg`} alt="..." height="600px" />
                    <div class="bg-dark card-img-overlay text-white d-flex flex-column justify-content-center storeOpacity">
                        ...
                        <img
                          ...
                          onClick={() => {
                            ...
                            let body = {
                              ...
                              위치: foodCago.곱창[i].위치,
                              특징: foodCago.곱창[i].특징,
                              평균가격: foodCago.곱창[i].가격,
                              좋아요: true,
                              삭제용: sessionStorage.getItem("ID") + foodCago.곱창[i].식당,
                              drink: "소주",
                              종류: "곱창",
                              사진: "/assets/0/0.jpg",
                            };
                            ...
                            {
                              !내거.find((e) => e.식당 === foodCago.곱창[i].식당) ? (alert("해당 상품이 마이페이지 찜목록에 추가 되었습니다."), axios.post("/selection", body).then(내거.push(body))) : alert("이미 찜목록에 존재하므로 찜목록 담기가 불가합니다.");
                            }
                          }}
                      ...
                      <h5 class="card-title">{foodCago.곱창[i].식당}</h5>
                      <p class="card-text">
                        <strong>위치</strong> : {foodCago.곱창[i].위치}
                      </p>
                      <p class="card-text">
                        <strong>특징</strong> : {foodCago.곱창[i].특징}
                      </p>
                      <p class="card-text">
                        <strong>평균가격</strong> : {foodCago.곱창[i].가격}
                      </p>
                    </div>
                  </div>
                </div>
